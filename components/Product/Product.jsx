...
<div className='md:pt-[70px] lg:pt-[130px]'>
       ...
       <div className='lg:px-10 xl:bg-[#f6faff] xl:rounded-full xl:mx-10  '>
        <ul className=' mt-[50px] md:grid md:grid-cols-3 md:gap-5 lg:gap-8 px-10 text-center xl:grid-cols-6 xl:px-0 lg:items-center '>
       
        {categories.map((category) => (
          
           <div>
            {(category === "vps kvm" && <button key={category} onClick={() => setCategory(category) } className=' focus:bg-[#ff9800] active:bg-[#ff9800] text-slate-600 font-semibold bg-[#f6faff] cs:mt-4 mt-4 py-4 rounded-full hover:bg-slate-200 w-full cursor-pointer cs3:px-4 cs3:py-4 '>{(category === 'vps kvm' && <span>VPS KVM</span> )} <span className='bg-[#f43636] text-white py-1 px-2 rounded-md text-sm'>Baru</span>
           </button>)  || (category ?  <button key={category} onClick={() => setCategory(category)} className='focus:bg-[#ff9800] active:bg-[#ff9800] active:text-white focus:text-white transition-all ease-in duration-300 text-slate-600 font-semibold cs:mt-4 mt-4 py-4 rounded-full bg-[#f6faff] hover:bg-slate-200 w-full cursor-pointer cs3:px-4 cs3:py-4'>
    ...
    <div className=' lg:mr-0 '><h1 className='text-center text-3xl font-semibold px-5 mt-[85px] md:text-[#3A4166]  md:mt-[50px] lg:text-[37px]'>Unlimited Web Hosting Indonesia</h1></div>
        <div className=' lg:mr-0' >
        <p className='text-center text-md text-slate-500 px-10 mt-10 md:mt-5 lg:mx-[70px] md:px-[120px] '>Layanan web hosting Indonesia terbaik dengan fitur hosting terlengkap dan dukungan support 24 jam untuk kemudahan Anda meraih kesuksesan online!</p>
        </div>
